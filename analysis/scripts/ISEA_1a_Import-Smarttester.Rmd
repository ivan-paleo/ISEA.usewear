...
Due to the huge number of TXT files (`r 20*5*5*12`), it was not possible to upload them to GitHub. They were therefore stored and accessed locally in "`r dir_in`".  
...
samples_ID <- dirname(dirname(TXT_files)) %>% # Path of folder 2 levels higher
              basename() %>%                  # Name of folder 2 levels higher
              unique()                        # Unique names

# Extract folder name from paths
messung <- dirname(TXT_files) %>% # Path of folder 1 level higher
           basename()             # Name of folder 1 level higher

# Extract file name from paths
filen <- basename(TXT_files)
```
...
```{r}
# prepare for loop - To DELETE LATER
i <- 1

# Extract sensor's name and unit from the TXT file
                    # Info on Line 3
sensor_name_unit <- read.table(TXT_files[i], skip = 2, nrows = 1, sep = ";", fileEncoding = 'WINDOWS-1252') %>% 
                    ...
                    # Convert to vector to be used in comment() below
                    unlist()

# Rename vector
names(sensor_name_unit) <- c("Sensor", "Unit")

# Read in data
dataTXT <- read.table(TXT_files[i], skip = 4, sep = ";")
names(dataTXT) <- c(sensor_name_unit[["Sensor"]], "Step")
comment(dataTXT) <- c(sensor_name_unit, samples_ID[i], messung[i], filen[i])
```
