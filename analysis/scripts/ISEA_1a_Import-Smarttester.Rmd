...
With 20 files per folder and a folder for each combination of angle (5), sensor (5) and sample (12), a total of `r 20*5*5*12` TXT files were saved. Nevertheless, there were issues during the experiment with ISEA-EX3 so that the first 400 strokes with that sample were not usable. Still, `r 20*5*5*11 + 400` TXT files were used.
...
Due to the huge number of TXT files (`r 20*5*5*11 + 400`), it was not possible to upload them to GitHub. They were therefore stored and accessed locally in "`r dir_in`" for running the script. They can be accessed on Zenodo: *add DOI*
