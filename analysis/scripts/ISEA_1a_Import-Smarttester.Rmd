...
for (i in seq_along(Samples_ID)) {
  ...
  TXT_data[[j]] <- read.table(TXT_files[j], skip = 4, sep = ";") %>%
                   mutate(Sample = Samples_ID[i], File = smartfile, Bamboo_position = bamboo, Sensor_name = name_unit) %>%
                   #mutate(Sample = Samples_ID[i], Folder = messung, File = smartfile, Sensor_name = name_unit) %>%
                   #mutate(Sample = Samples_ID[i], File = smartfile, Sensor_name = name_unit) %>%
...
wide_data <- pivot_wider(all_data, names_from = Sensor_name, values_from = Sensor_value) %>% 
             mutate(Bamboo_position = factor(Bamboo_position, levels = unique(folders$bamboo)), Stroke = NA) %>% 
...
## Add stroke number
```{r}
wide_data[1, "Stroke"] <- 1
test <- group_by(wide_data, Sample) %>% 
    mutate(changes = c(0, diff(diff(`Stroke [mm]`)), 0) != 0)
View(test) 

x <- split(wide_data, wide_data$Sample)
x1 <- x[[1]]
x1[1:10, "Stroke [mm]"]
diff(x1[["Stroke [mm]"]])[1:12]
diff(diff(x1[["Stroke [mm]"]]))[1:12]
...
# Save data
```{r}
write_ods(all_data, path = paste0(dir_out, "/ISEA_use-wear_all-data.ods"))
write_ods(wide_data, path = paste0(dir_out, "/ISEA_use-wear_wide-data.ods"))
```
